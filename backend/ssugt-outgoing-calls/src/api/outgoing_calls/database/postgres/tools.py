...
def sa_inject_holder(session_factory: async_sessionmaker[AsyncSession]):

    @asynccontextmanager
    async def wrapper() -> AsyncGenerator[DatabasePostgresHolder, None]:
        async with session_factory() as session:
            yield DatabasePostgresHolder(session=session)
